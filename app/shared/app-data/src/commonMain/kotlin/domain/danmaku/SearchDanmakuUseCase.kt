...
fun interface SearchDanmakuUseCase : UseCase {
    ...
    suspend operator fun invoke(request: SearchDanmakuRequest): List<DanmakuFetchResult>
}
...
class SearchDanmakuUseCaseImpl(
    ...
    override suspend fun invoke(request: SearchDanmakuRequest): List<DanmakuFetchResult> = withContext(context) {
        repo.search(request)
    }
}
