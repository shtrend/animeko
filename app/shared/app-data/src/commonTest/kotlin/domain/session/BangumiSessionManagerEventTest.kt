...
class BangumiSessionManagerEventTest : AbstractBangumiSessionManagerTest() {
    ...
    fun `setSession to AccessToken emits event`() = runTest {
        ...
        runCollectingEvents(manager) {
            setSession(AccessTokenSession(AccessTokenPair(ACCESS_TOKEN, ACCESS_TOKEN, expiresAtMillis = 1), 1))
        }.run {
            assertEquals(1, size)
    ...
    fun `clearSession emits LogOut when session is AccessToken`() = runTest {
        ...
        manager.setSession(AccessTokenSession(AccessTokenPair(ACCESS_TOKEN, ACCESS_TOKEN, expiresAtMillis = 1), 1))
        runCollectingEvents(manager) {
            clearSession()
        }.run {
            assertEquals(1, size)
